...
const variantStyles = {
  default: {
    ...
    th: 'text-left px-4 py-3 font-semibold text-gray-600',
  ...
  ghost: {
    container: '',
    thead: 'bg-[#fdfdfd]',
    th: 'text-left px-6 py-2 font-semibold text-gray-500 text-xs',
...
const actionBase = 'inline-flex items-center gap-1 font-medium transition focus:outline-none';
const actionVariantClasses: Record<string, string> = {
  neutral: 'bg-gray-100 text-gray-700 hover:bg-gray-200 rounded px-3 py-1.5 text-xs',
...
function currencySymbol(code: string): string {
  const map: Record<string, string> = {
    MYR: 'RM', USD: '$', JPY: '¥', CNY: '¥', THB: '฿',
    EUR: '€', GBP: '£', SGD: 'S$', KRW: '₩', AUD: 'A$',
    CAD: 'C$', INR: '₹', VND: '₫', PHP: '₱', IDR: 'Rp',
    HKD: 'HK$', TWD: 'NT$',
  };
      ...
      {actions.map((action, i) => {
          ...
          return (
            <form
              ...
              onSubmit={
                action.confirm
                  ? (e) => {
                      if (!confirm(replaceParams(action.confirm)!)) e.preventDefault();
                    }
                  : undefined
              }
            >
        ...
        const handleClick =
          action.onClick && registry?.[action.onClick]
            ? () => {
                const handler = action.onClick ? registry[action.onClick] : undefined;
                handler?.(row);
              }
            : undefined;

        return (
          <button key={i} type="button" className={className} onClick={handleClick}>
            {content}
          </button>
        );
      })}
...
> = {
  text: (value) => (value === undefined || value === null || value === '' ? '-' : String(value)),
  date: (value) => {
  ...
  currency: (_value, row, meta) => {
    ...
    const prefSymbol = String(meta?.currencySymbol ?? currencySymbol(prefCurrency));

    return (
      <div>
        <div className="font-medium">{formatCurrency(converted, prefSymbol)}</div>
  ...
  actions: (_value, row, _meta, column, registry) => {
    ...
    return <ActionCell row={row} actions={column.actions} registry={registry} />;
  },
};

export function DataTable({ columns, data, variant = 'default', meta, actions: registry }: DataTableProps) {
