...
**Write the map and every ticket to the shapes in [`FORMAT.md`](FORMAT.md), and verify against its checklist before committing.** It holds the map body, the ticket frontmatter, the fog-patch shape, and the seven checks.
...
A session **claims** a ticket by setting `status: claimed` with `claimed_by` and `claimed_at`, and saving, **first**, before any work, so concurrent sessions skip it. Commit the claim promptly — on a shared repo, other clones only see it once it lands. The timestamp is what tells a live session apart from one that died mid-ticket; without it the frontier steps around that ticket forever.
...
`out_of_scope` is closed, and closed is not resolved: it satisfies no `blocked_by` edge. A ticket blocked by an out-of-scope ticket therefore never unblocks — one of the two is mis-scoped, and you should say which.

### Undermined decisions

A resolved ticket sometimes rests on a premise that a later ticket destroys. Mark it and say what broke: add `undermined_by: [NN]` naming the ticket that broke it, and open its `## Answer` with a line saying so. The decision still stands — nobody has reopened it — but every session that reads the map can now see what it is standing on. A decision recorded as simply `resolved` reads as settled, and that is how a live problem gets laundered into a checkmark.
...
Each patch carries a **title** and, where you know which open ticket will clear it, an **anchor** — see [`FORMAT.md`](FORMAT.md). A title names a question no live ticket holds; a question tracked both sharply and vaguely rots in its vague copy.
...
4. **Create the tickets you can specify now** as files under `tickets/` — then wire the `blocked_by` lists in a **second pass** (tickets need numbers before they can reference each other). Wiring sorts them into the frontier and the blocked; everything you can't yet specify stays in the fog — the **Not yet specified** section.
5. **Verify** against the checklist in [`FORMAT.md`](FORMAT.md) before committing.
...
6. **Verify** against the checklist in [`FORMAT.md`](FORMAT.md) before committing.
