...
The frontier is two unblocked tickets, and they are independent. **Claim a ticket, and
commit the claim, before doing any work.** (Both the ticket format and the concurrency
rule have changed since this handoff was written — the markdown adapter now permits only
one session at a time. The skill and its `TRACKER-MARKDOWN.md` adapter are authoritative
on both.)
